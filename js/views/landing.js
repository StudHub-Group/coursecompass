...
async function finishSignup(instKey,opts){
  ...
  // Optional, non-blocking — Firebase signs the user in immediately (unlike
  // Supabase's optional "confirm your email" gate), so this is just a nudge.
...
function renderAuthCheckEmail(){
  ...
  return '<div class="flex flex-col items-center space-y-3 py-4 text-center">'+
    ...
    '<button class="btn btn-outline btn-sm mt-2" data-action="back-to-signin">'+t('auth.check_email_back')+'</button></div>';
}
...
function bindLanding(){
  ...
  if(bt3) bt3.addEventListener('click',()=>{state.signup.step='form';state.tab='signin';render()});
